...
class FDA():
    ...
    def __init__(self, Order = 4, data=[], PlotBasis=False):
        ...
        i, j = 0, 0
        temp = lambda x, y, j=j, i=i : Legendre(j,x)/np.sqrt(2/(2*j+1)) * Legendre(i-j,y)/np.sqrt(2/(2*(i-j)+1))
    ...
    def Plot_Basis(self):
        ...
        plt.savefig("G:\gitCode\BU_programming\\finnal_project\FuNVol\image\\basi_function.png")
        plt.show()        
    ...
    def Fit_IV_FPC(self, X, Y, Z, K, PlotFit=False):
        ...
        for i in range(K):
            basis_value[:,i] = self.phi[i](X_flat, Y_flat)
    ...
    def Compute_FPC_coeff(self, a, PlotFPC=True, savefig=False, save_path="image/fpcs.png"):
